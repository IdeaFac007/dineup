...
type Order={id:number;order_number:string;restaurant_id:number;status:string;payment_status:string;fulfillment_type:string;subtotal:number;discount_amount:number;delivery_fee:number;total_amount:number;customer_note:string|null;cancellation_reason:string|null;refund_status:string|null;refund_amount:number;refunded_at:string|null;created_at:string;updated_at:string};
...
export default function CustomerOrdersPage(){
 ...
 const load=useCallback(async(refresh=false)=>{
  ...
  try{
   ...
   const {data:o,error:oe}=await supabase.from("orders").select("id,order_number,restaurant_id,status,payment_status,fulfillment_type,subtotal,discount_amount,delivery_fee,total_amount,customer_note,cancellation_reason,refund_status,refund_amount,refunded_at,created_at,updated_at").eq("customer_id",user.id).order("created_at",{ascending:false}).limit(50);
   if(oe)throw oe;
   const normalized=(o||[]).map((x:any)=>({...x,id:Number(x.id),restaurant_id:Number(x.restaurant_id),subtotal:Number(x.subtotal||0),discount_amount:Number(x.discount_amount||0),delivery_fee:Number(x.delivery_fee||0),total_amount:Number(x.total_amount||0),refund_amount:Number(x.refund_amount||0)})) as Order[];
 ...
 async function cancelOrder(order:Order){
  if(!window.confirm("Cancel this order? If it is already paid, DineUp will request a full Razorpay refund."))return;
  setError("");setMessage("");setPaying(order.id);
  try{
   const r=await fetch("/api/orders/cancel",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({orderId:order.id,reason:"Cancelled by customer"})});
   const d=await r.json();if(!r.ok||!d.success)throw new Error(d.error||"Unable to cancel order.");
   setMessage(d.refund?.status==="processed"?"Order cancelled. Full refund processed.":d.refund?"Order cancelled. Refund requested; payment status will update when processed.":"Order cancelled.");
   await load(true);
  }catch(e:any){setError(e.message||"Unable to cancel order.")}finally{setPaying(null)}
 }
