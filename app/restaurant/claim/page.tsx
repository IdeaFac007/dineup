...
export default function ClaimFinder(){
  ...
  const [q,setQ]=useState(""); const [city,setCity]=useState(""); const [googleMapsUrl,setGoogleMapsUrl]=useState("");
  ...
  return <main className="page">
    ...
    <section className="box">
      ...
      {loading?<div className="empty">Loading listings…</div>:filtered.length?<div className="list">{filtered.map(r=>{
        ...
        return <div className="item" key={r.id}><div><b>{r.name}</b><span>{r.category||"Restaurant"} · {r.city}{r.address?" · "+r.address:""}</span></div>{unavailable?<em>Already claimed / under review</em>:<Link href={"/restaurant/claim/"+r.id+(googleMapsUrl.trim()?"?google_maps_url="+encodeURIComponent(googleMapsUrl.trim()):"")}>Claim listing →</Link>}</div>
