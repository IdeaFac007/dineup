...
export default function PublicRestaurantProfile(){
 ...
 const errorRef=useRef<HTMLDivElement|null>(null); const profileViewTrackedRef=useRef<number|null>(null); const [restaurant,setRestaurant]=useState<Restaurant|null>(null); const [profile,setProfile]=useState<Profile|null>(null); const [gallery,setGallery]=useState<Media[]>([]); const [rank,setRank]=useState<number|null>(null); const [loading,setLoading]=useState(true); const [error,setError]=useState(""); const [shareMessage,setShareMessage]=useState(""); const [addressMessage,setAddressMessage]=useState(""); const [phoneMessage,setPhoneMessage]=useState(""); const [todayKey,setTodayKey]=useState(""); const [brokenCover,setBrokenCover]=useState(false); const [brokenLogo,setBrokenLogo]=useState(false); const [brokenGallery,setBrokenGallery]=useState<number[]>([]);
