...
export default function PublicRestaurantProfile(){
 ...
 useEffect(()=>{ let cancelled=false; async function load(){ actionTrackRef.current={}; setLoading(true); setRestaurant(null); setProfile(null); setGallery([]); setRank(null); setBrokenCover(false); setBrokenLogo(false); setBrokenGallery([]); setError(""); setShareMessage(""); setAddressMessage(""); setPhoneMessage(""); if(!Number.isFinite(id)||id<=0){if(!cancelled){setError("Restaurant not found.");setLoading(false);}return;} setTodayKey(new Intl.DateTimeFormat("en-IN",{weekday:"long",timeZone:"Asia/Kolkata"}).format(new Date()).toLowerCase()); try{
   ...
   if(r.city?.trim()){
     const cityPattern = r.city.trim().replace(/[%_\\]/g, "\\     const cityPattern=r.city.trim().replace(/[\\%_]/g,"\\     const {data:cityList,error:ce}=await supabase.from("restaurants").select("id,current_bid").ilike("city",r.city.trim()).eq("is_active",true).order("current_bid",{ascending:false,nullsFirst:false}).order("id",{ascending:true});"); const {data:cityList,error:ce}=await supabase.from("restaurants").select("id,current_bid").ilike("city",cityPattern).eq("is_active",true).order("current_bid",{ascending:false,nullsFirst:false}).order("id",{ascending:true});");
     const {data:cityList,error:ce}=await supabase.from("restaurants").select("id,current_bid").ilike("city",cityPattern).eq("is_active",true).order("current_bid",{ascending:false,nullsFirst:false}).order("id",{ascending:true});
     if(!cancelled){
