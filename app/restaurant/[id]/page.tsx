...
export default function PublicRestaurantProfile(){
 ...
 const [restaurant,setRestaurant]=useState<Restaurant|null>(null); const [profile,setProfile]=useState<Profile|null>(null); const [gallery,setGallery]=useState<Media[]>([]); const [rank,setRank]=useState<number|null>(null); const [loading,setLoading]=useState(true); const [error,setError]=useState(""); const [shareMessage,setShareMessage]=useState("");
 ...
 return <main className="page"><Header/><section className="shell"><div className="cover" style={profile?.cover_image_url?{backgroundImage:`url(${profile.cover_image_url})`}:undefined}>{!profile?.cover_image_url&&<div className="cover-letter">{restaurant.name[0]}</div>}<div className="coverShade"/><div className="rankBadge">{rank?`#${rank} in ${restaurant.city}`:"DineUp listing"}</div></div>
 <div className="main"><div className="identity"><div className="logo">{profile?.logo_image_url?<Image src={profile.logo_image_url} alt={`${restaurant.name} logo`} width={184} height={184} sizes="92px" loading="eager" unoptimized />:restaurant.name[0]}</div><div><small>RESTAURANT</small><h1>{restaurant.name}</h1><p>{restaurant.category} • {restaurant.city}</p><div className="badges">{sponsored&&<b>SPONSORED</b>}{restaurant.claim_status==="verified"&&<b className="verifiedBadge">✓ DINEUP VERIFIED</b>}{restaurant.is_claimed&&restaurant.claim_status!=="verified"&&<b className="claimed">✓ CLAIMED</b>}{restaurant.claim_status==="verification_pending"&&<b className="pending">VERIFICATION PENDING</b>}</div></div><div className="rankCard"><small>DINEUP RANK</small><strong>{rank?`#${rank}`:"—"}</strong><span>{restaurant.city}</span></div></div>
 <div className="actions"><button type="button" className="secondary shareButton" onClick={()=>void shareProfile()}>Share profile</button>{phone&&<a className="primary" href={`tel:${phone}`} onClick={()=>void track("call")}>Call restaurant</a>}{whatsappNumber&&<a className="secondary" href={`https://wa.me/${whatsappNumber}?text=${whatsappMessage}`} target="_blank" rel="noopener noreferrer" aria-label={`Chat with ${restaurant.name} on WhatsApp`} onClick={()=>void track("whatsapp")}>WhatsApp</a>}<a className="secondary" href={maps} target="_blank" rel="noreferrer" onClick={()=>void track("directions")}>Get directions</a>{profile?.menu_url&&<a className="secondary" href={profile.menu_url} target="_blank" rel="noreferrer" onClick={()=>void track("menu")}>View menu</a>}{restaurant.claim_status==="unclaimed"&&!restaurant.is_claimed&&<Link className="secondary" href={"/restaurant/claim/"+id}>Claim this restaurant</Link>}{profile?.website_url&&<a className="secondary" href={profile.website_url} target="_blank" rel="noreferrer" onClick={()=>void track("website")}>Website</a>}</div>{shareMessage&&<div className="shareNotice" role="status" aria-live="polite">{shareMessage}</div>}
 ...
 {gallery.length>0&&<section className="card"><small>PHOTOS</small><h2>Restaurant gallery</h2><div className="publicGallery">{gallery.map(item=><figure className="publicGalleryItem" key={item.id}><Image src={item.public_url} alt={item.title||restaurant.name} width={800} height={600} sizes="(max-width: 520px) 100vw, (max-width: 800px) 50vw, 33vw" loading="lazy" unoptimized />{item.caption&&<figcaption>{item.caption}</figcaption>}</figure>)}</div></section>}
 <section className="card"><small>LOCATION</small><h2>Find us</h2><div className="location"><div>⌖</div><p><strong>{restaurant.city}</strong><br/>{restaurant.address||"Address available on request."}</p></div></section>
