...
export default function PublicRestaurantProfile(){
 ...
 <div className="actions"><button type="button" className="secondary shareButton" aria-label={`Share ${restaurant.name} profile`} title={`Share ${restaurant.name} profile`} onClick={()=>void shareProfile()}>Share profile</button>{phoneHref&&<a className="primary" href={`tel:${phoneHref}`} onClick={()=>void track("call")}>Call restaurant</a>}{whatsappNumber&&<a className="secondary" href={`https://wa.me/${whatsappNumber}?text=${whatsappMessage}`} target="_blank" rel="noopener noreferrer" aria-label={`Chat with ${restaurant.name} on WhatsApp`} onClick={()=>void track("whatsapp")}>WhatsApp</a>}<a className="secondary" href={maps} target="_blank" rel="noopener noreferrer" aria-label={`Get directions to ${restaurant.name}`} onClick={()=>void track("directions")}>Get directions</a>{menuUrl&&<a className="secondary" href={menuUrl} target="_blank" rel="noopener noreferrer" aria-label={`View ${restaurant.name} menu`} onClick={()=>void track("menu")}>View menu</a>}{restaurant.claim_status==="unclaimed"&&!restaurant.is_claimed&&<Link className="secondary" href={"/restaurant/claim/"+id}>Claim this restaurant</Link>}{websiteUrl&&<a className="secondary" href={websiteUrl} target="_blank" rel="noopener noreferrer" aria-label={`Visit ${restaurant.name} website`} onClick={()=>void track("website")}>Website</a>}{instagramUrl&&<a className="secondary" href={instagramUrl} target="_blank" rel="noopener noreferrer" aria-label={`View ${restaurant.name} on Instagram`}>Instagram</a>}</div>{shareMessage&&<div className="shareNotice" role="status" aria-live="polite">{shareMessage}</div>}
