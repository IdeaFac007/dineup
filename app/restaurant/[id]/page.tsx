...
export default function PublicRestaurantProfile(){
 ...
 if(!restaurant||error)return <><Header/><div className="state" role="alert" aria-labelledby="profile-error-title" aria-describedby="profile-error-message"><span>DINEUP</span><h1 id="profile-error-title">Restaurant unavailable</h1><p id="profile-error-message">{error||"Restaurant not found."}</p><div className="stateActions"><button type="button" className="primary stateRetry" onClick={()=>{setLoading(true);setError("");window.location.reload();}}>Retry</button><Link href="/marketplace" className="secondary">Back to marketplace</Link></div></div><style jsx global>{styles}</style></>;
