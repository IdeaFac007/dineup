...
export default function PublicRestaurantProfile(){
 ...
 <section className="card">{gallery.length>0&&<><small>PHOTOS</small><h2>Restaurant gallery</h2><div className="publicGallery">{gallery.map(item=><figure className="publicGalleryItem" key={item.id}><img src={item.public_url} alt={item.title||restaurant.name} loading="lazy" decoding="async"/>{item.caption&&<figcaption>{item.caption}</figcaption>}</figure>)}</div></>}</section>
