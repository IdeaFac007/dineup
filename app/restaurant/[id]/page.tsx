...
export default function PublicRestaurantProfile(){
 ...
 const errorRef=useRef<HTMLDivElement|null>(null); const [restaurant,setRestaurant]=useState<Restaurant|null>(null); const [profile,setProfile]=useState<Profile|null>(null); const [gallery,setGallery]=useState<Media[]>([]); const [rank,setRank]=useState<number|null>(null); const [loading,setLoading]=useState(true); const [error,setError]=useState(""); const [shareMessage,setShareMessage]=useState(""); const [addressMessage,setAddressMessage]=useState(""); const [phoneMessage,setPhoneMessage]=useState(""); const [todayKey,setTodayKey]=useState(""); const [brokenCover,setBrokenCover]=useState(false); const [brokenLogo,setBrokenLogo]=useState(false); const [brokenGallery,setBrokenGallery]=useState<number[]>([]);
 ...
 useEffect(()=>{ let cancelled=false; async function load(){ setLoading(true); setRestaurant(null); setProfile(null); setGallery([]); setRank(null); setBrokenCover(false); setBrokenLogo(false); setBrokenGallery([]); setError(""); setShareMessage(""); setAddressMessage(""); setPhoneMessage(""); if(!Number.isFinite(id)||id<=0){if(!cancelled){setError("Restaurant not found.");setLoading(false);}return;} setTodayKey(new Intl.DateTimeFormat("en-IN",{weekday:"long",timeZone:"Asia/Kolkata"}).format(new Date()).toLowerCase()); try{
   ...
   const {data:cityList,error:ce}=await supabase.from("restaurants").select("id,current_bid").eq("city",r.city).eq("is_active",true).order("current_bid",{ascending:false}).order("id",{ascending:true});
   if(!cancelled){
 ...
 useEffect(()=>{ const timers:number[]=[]; if(shareMessage)timers.push(window.setTimeout(()=>setShareMessage(""),3500)); if(addressMessage)timers.push(window.setTimeout(()=>setAddressMessage(""),3500)); if(phoneMessage)timers.push(window.setTimeout(()=>setPhoneMessage(""),3500)); return()=>timers.forEach(window.clearTimeout); },[shareMessage,addressMessage,phoneMessage]);
