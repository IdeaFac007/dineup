...
export default function PublicRestaurantProfile(){
 ...
 const errorRef=useRef<HTMLDivElement|null>(null); const profileViewTrackedRef=useRef(false); const [restaurant,setRestaurant]=useState<Restaurant|null>(null); const [profile,setProfile]=useState<Profile|null>(null); const [gallery,setGallery]=useState<Media[]>([]); const [rank,setRank]=useState<number|null>(null); const [loading,setLoading]=useState(true); const [error,setError]=useState(""); const [shareMessage,setShareMessage]=useState(""); const [addressMessage,setAddressMessage]=useState(""); const [phoneMessage,setPhoneMessage]=useState(""); const [todayKey,setTodayKey]=useState(""); const [brokenCover,setBrokenCover]=useState(false); const [brokenLogo,setBrokenLogo]=useState(false); const [brokenGallery,setBrokenGallery]=useState<number[]>([]);
 ...
 useEffect(()=>{ let cancelled=false; async function load(){ profileViewTrackedRef.current=false; setLoading(true); setRestaurant(null); setProfile(null); setGallery([]); setRank(null); setBrokenCover(false); setBrokenLogo(false); setBrokenGallery([]); setError(""); setShareMessage(""); setAddressMessage(""); setPhoneMessage(""); if(!Number.isFinite(id)||id<=0){if(!cancelled){setError("Restaurant not found.");setLoading(false);}return;} setTodayKey(new Intl.DateTimeFormat("en-IN",{weekday:"long",timeZone:"Asia/Kolkata"}).format(new Date()).toLowerCase()); try{
   ...
   if(!cancelled&&!profileViewTrackedRef.current){ profileViewTrackedRef.current=true; void track("profile_view"); }
