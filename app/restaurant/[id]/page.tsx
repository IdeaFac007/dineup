...
function normalizeOpeningHours(value:Record<string,string>|null|undefined){
 ...
 for(const [day,hours] of Object.entries(value||{})){
   ...
   const clean=typeof hours==="string"?hours.trim():"";
   if(!DAYS.some(([dayKey])=>dayKey===key)||!clean||normalized[key])continue;
...
export default function PublicRestaurantProfile(){
 ...
 useEffect(()=>{ let cancelled=false; async function load(){ actionTrackRef.current={}; setLoading(true); setRestaurant(null); setProfile(null); setGallery([]); setRank(null); setBrokenCover(false); setBrokenLogo(false); setBrokenGallery([]); setError(""); setShareMessage(""); setAddressMessage(""); setPhoneMessage(""); if(!Number.isFinite(id)||id<=0){if(!cancelled){setError("Restaurant not found.");setLoading(false);}return;} setTodayKey(new Intl.DateTimeFormat("en-IN",{weekday:"long",timeZone:"Asia/Kolkata"}).format(new Date()).toLowerCase()); try{
   ...
   const currentBid=Number(r.current_bid);
   const normalizedCurrentBid=Number.isFinite(currentBid)&&currentBid>0?currentBid:0;
   const normalizedClaimStatus=normalizeClaimStatus(r.claim_status);
   const normalizedIsClaimed=r.is_claimed===true||normalizedClaimStatus==="verified"||normalizedClaimStatus==="claimed";
   setRestaurant({...r,id:Number(r.id),name:String(r.name||"").trim()||"Restaurant",city:String(r.city||"").trim(),category:String(r.category||"").trim()||"Restaurant",address:normalizeOptionalText(r.address),current_bid:normalizedCurrentBid,is_claimed:normalizedIsClaimed,claim_status:normalizedClaimStatus});
   const [{data:p,error:pe},{data:media,error:me}]=await Promise.all([supabase.from("restaurant_profiles").select("phone,whatsapp,website_url,description,price_range,menu_url,cover_image_url,logo_image_url,opening_hours,instagram_url,google_maps_url,cuisine_tags").eq("restaurant_id",id).maybeSingle(),supabase.from("restaurant_media").select("id,media_type,public_url,title,caption,sort_order").eq("restaurant_id",id).eq("media_type","gallery").eq("is_active",true).order("sort_order",{ascending:true}).order("created_at",{ascending:true})]); if(pe)throw pe; if(me)throw me; if(cancelled)return; const normalizedProfile=p?{...p,phone:normalizeOptionalText(p.phone),whatsapp:normalizeOptionalText(p.whatsapp),description:normalizeOptionalText(p.description),price_range:normalizeOptionalText(p.price_range),cover_image_url:safeRestaurantMediaUrl(p.cover_image_url)||null,logo_image_url:safeRestaurantMediaUrl(p.logo_image_url)||null,cuisine_tags:normalizeCuisineTags(p.cuisine_tags),opening_hours:normalizeOpeningHours(p.opening_hours)}:null; const normalizedGallery=(media||[]).map(item=>({...item,id:Number(item.id),sort_order:Number(item.sort_order)||0,title:normalizeOptionalText(item.title),caption:normalizeOptionalText(item.caption),public_url:safeRestaurantMediaUrl(item.public_url)})).filter(item=>Number.isFinite(item.id)&&item.id>0&&Boolean(item.public_url)).sort((a,b)=>a.sort_order-b.sort_order||a.id-b.id).filter((item,index,items)=>items.findIndex(candidate=>candidate.id===item.id)===index).filter((item,index,items)=>items.findIndex(candidate=>candidate.public_url===item.public_url)===index); setProfile(normalizedProfile as Profile|null); setGallery(normalizedGallery as Media[]);
   if(r.city?.trim()){
     const cityName=r.city.trim().toLocaleLowerCase("en-IN");
     const {data:cityList,error:ce}=await supabase.from("restaurants").select("id,current_bid,city").ilike("city",r.city.trim()).eq("is_active",true).order("current_bid",{ascending:false,nullsFirst:false}).order("id",{ascending:true});
     if(!cancelled){
       if(ce){console.warn("Restaurant ranking lookup failed:",ce.message);setRank(null);}else{const exactCityList=(cityList||[]).filter(x=>String(x.city||"").trim().toLocaleLowerCase("en-IN")===cityName); const position=exactCityList.findIndex(x=>Number(x.id)===id);setRank(position>=0?position+1:null);}
     }
   }else if(!cancelled){setRank(null);}
 ...
 const restaurantInitial=(restaurant.name||"D").trim().charAt(0).toUpperCase()||"D";
 const restaurantCategory=(restaurant.category||"Restaurant").trim()||"Restaurant";
 const restaurantCityLabel=(restaurant.city||"").trim()||"City not provided";
 ...
 const shareProfile=async()=>{
   ...
   const text=restaurant.city?.trim()?`Discover ${restaurant.name} in ${restaurant.city.trim()} on DineUp.`:`Discover ${restaurant.name} on DineUp.`;
   setShareMessage("");
   try{
 ...
 const structuredData: Record<string, unknown>={"@context":"https://schema.org","@type":"Restaurant","name":restaurant.name,"url":"https://dineupindia.com/restaurant/"+restaurant.id,"address":profile?.address?{"@type":"PostalAddress","streetAddress":profile.address,"addressLocality":restaurant.city,"addressCountry":"IN"}:{"@type":"PostalAddress","addressLocality":restaurant.city,"addressCountry":"IN"},"servesCuisine":normalizeCuisineTags(profile?.cuisine_tags),"telephone":profile?.phone||undefined,"description":profile?.description||undefined,"image":[profile?.cover_image_url,profile?.logo_image_url].filter(Boolean)};
 return <main className="page"><script id="restaurant-jsonld" type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(structuredData) }} /><Header/><section className="shell"><nav className="breadcrumb" aria-label="Breadcrumb"><Link href="/marketplace">Marketplace</Link><span aria-hidden="true">/</span><span>{restaurantCityLabel}</span><span aria-hidden="true">/</span><span aria-current="page">{restaurant.name}</span></nav><div className="cover">{profile?.cover_image_url&&!brokenCover?<Image className="coverImage" src={profile.cover_image_url} alt={`${restaurant.name} cover`} fill priority sizes="(max-width: 800px) 100vw, 1180px" referrerPolicy="no-referrer" onError={()=>setBrokenCover(true)} />:<div className="cover-letter">{restaurantInitial}</div>}<div className="coverShade"/><div className="rankBadge">{rank?`#${rank} in ${restaurantCityLabel}`:"DineUp listing"}</div></div>
 <div className="main"><div className="identity"><div className="logo">{profile?.logo_image_url&&!brokenLogo?<Image src={profile.logo_image_url} alt={`${restaurant.name} logo`} width={184} height={184} sizes="92px" loading="eager" referrerPolicy="no-referrer" onError={()=>setBrokenLogo(true)} />:restaurant.name[0]}</div><div><small>RESTAURANT</small><h1>{restaurant.name}</h1><p>{restaurantCategory} • {restaurantCityLabel}</p><div className="badges">{sponsored&&<b>SPONSORED</b>}{restaurant.claim_status==="verified"&&<b className="verifiedBadge">✓ DINEUP VERIFIED</b>}{restaurant.is_claimed&&restaurant.claim_status!=="verified"&&<b className="claimed">✓ CLAIMED</b>}{restaurant.claim_status==="verification_pending"&&<b className="pending">VERIFICATION PENDING</b>}</div></div><div className="rankCard" role="group" aria-label={`${restaurant.name} DineUp ranking`}><small>DINEUP RANK</small><strong>{rank?`#${rank}`:"—"}</strong><span>{restaurantCityLabel}</span></div></div>
