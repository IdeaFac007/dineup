...
export default function PublicRestaurantProfile(){
 ...
 useEffect(()=>{ async function load(){ setBrokenCover(false); setBrokenLogo(false); setBrokenGallery([]); setError(""); setShareMessage(""); setAddressMessage(""); setPhoneMessage(""); if(!Number.isFinite(id)||id<=0){setError("Restaurant not found.");setLoading(false);return;} setTodayKey(new Intl.DateTimeFormat("en-IN",{weekday:"long",timeZone:"Asia/Kolkata"}).format(new Date()).toLowerCase()); try{
 ...
 {gallery.some(item=>!brokenGallery.includes(item.id))&&<section className="card"><small>PHOTOS</small><h2>Restaurant gallery</h2><div className="publicGallery">{gallery.filter(item=>!brokenGallery.includes(item.id)).map(item=><figure className="publicGalleryItem" key={item.id}><a className="publicGalleryLink" href={item.public_url} target="_blank" rel="noopener noreferrer" referrerPolicy="no-referrer" aria-label={`Open ${item.title||restaurant.name} photo in full size`}><Image src={item.public_url} alt={item.title||item.caption||`${restaurant.name} gallery photo`} width={800} height={600} sizes="(max-width: 520px) 100vw, (max-width: 800px) 50vw, 33vw" loading="lazy" referrerPolicy="no-referrer" onError={()=>setBrokenGallery(ids=>ids.includes(item.id)?ids:[...ids,item.id])} /></a>{item.caption&&<figcaption>{item.caption}</figcaption>}</figure>)}</div></section>}
 <section className="card"><small>LOCATION</small><h2>Find us</h2><div className="location"><div aria-hidden="true">⌖</div><p><strong>{restaurant.city}</strong><br/>{restaurant.address||"Address available on request."}</p><div className="locationActions"><a className="secondary locationAction" href={maps} target="_blank" rel="noopener noreferrer" referrerPolicy="no-referrer" onClick={()=>void track("directions")}>Directions</a>{addressText&&<button type="button" className="secondary locationAction" aria-label={`Copy ${restaurant.name} address`} title={`Copy ${restaurant.name} address`} onClick={()=>void copyAddress()}>Copy address</button>}</div></div>{addressMessage&&<div className="addressNotice" role="status" aria-live="polite">{addressMessage}</div>}</section>
