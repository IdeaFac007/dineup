...
export default function PublicRestaurantProfile(){
 ...
 return <main className="page"><Header/><section className="shell"><nav className="breadcrumb" aria-label="Breadcrumb"><Link href="/marketplace">Marketplace</Link><span aria-hidden="true">/</span><span>{restaurant.city}</span><span aria-hidden="true">/</span><span aria-current="page">{restaurant.name}</span></nav><div className="cover">{profile?.cover_image_url&&!brokenCover?<Image className="coverImage" src={profile.cover_image_url} alt={`${restaurant.name} cover`} fill priority sizes="(max-width: 800px) 100vw, 1180px" onError={()=>setBrokenCover(true)} />:<div className="cover-letter">{restaurant.name[0]}</div>}<div className="coverShade"/><div className="rankBadge">{rank?`#${rank} in ${restaurant.city}`:"DineUp listing"}</div></div>
 <div className="main"><div className="identity"><div className="logo">{profile?.logo_image_url&&!brokenLogo?<Image src={profile.logo_image_url} alt={`${restaurant.name} logo`} width={184} height={184} sizes="92px" loading="eager" onError={()=>setBrokenLogo(true)} />:restaurant.name[0]}</div><div><small>RESTAURANT</small><h1>{restaurant.name}</h1><p>{restaurant.category} • {restaurant.city}</p><div className="badges">{sponsored&&<b>SPONSORED</b>}{restaurant.claim_status==="verified"&&<b className="verifiedBadge">✓ DINEUP VERIFIED</b>}{restaurant.is_claimed&&restaurant.claim_status!=="verified"&&<b className="claimed">✓ CLAIMED</b>}{restaurant.claim_status==="verification_pending"&&<b className="pending">VERIFICATION PENDING</b>}</div></div><div className="rankCard"><small>DINEUP RANK</small><strong>{rank?`#${rank}`:"—"}</strong><span>{restaurant.city}</span></div></div>
 <div className="actions"><button type="button" className="secondary shareButton" aria-label={`Share ${restaurant.name} profile`} title={`Share ${restaurant.name} profile`} onClick={()=>void shareProfile()}>Share profile</button>{phoneHref&&<a className="primary" href={`tel:${phoneHref}`} onClick={()=>void track("call")}>Call restaurant</a>}{whatsappNumber&&<a className="secondary" href={`https://wa.me/${whatsappNumber}?text=${whatsappMessage}`} target="_blank" rel="noopener noreferrer" referrerPolicy="no-referrer" aria-label={`Chat with ${restaurant.name} on WhatsApp`} onClick={()=>void track("whatsapp")}>WhatsApp</a>}<a className="secondary" href={maps} target="_blank" rel="noopener noreferrer" referrerPolicy="no-referrer" aria-label={`Get directions to ${restaurant.name}`} onClick={()=>void track("directions")}>Get directions</a>{menuUrl&&<a className="secondary" href={menuUrl} target="_blank" rel="noopener noreferrer" referrerPolicy="no-referrer" aria-label={`View ${restaurant.name} menu`} onClick={()=>void track("menu")}>View menu</a>}{restaurant.claim_status==="unclaimed"&&!restaurant.is_claimed&&<Link className="secondary" href={"/restaurant/claim/"+id}>Claim this restaurant</Link>}{websiteUrl&&<a className="secondary" href={websiteUrl} target="_blank" rel="noopener noreferrer" referrerPolicy="no-referrer" aria-label={`Visit ${restaurant.name} website`} onClick={()=>void track("website")}>Website</a>}{instagramUrl&&<a className="secondary" href={instagramUrl} target="_blank" rel="noopener noreferrer" referrerPolicy="no-referrer" aria-label={`View ${restaurant.name} on Instagram`}>Instagram</a>}</div>{shareMessage&&<div className="shareNotice" role="status" aria-live="polite">{shareMessage}</div>}
 ...
 {gallery.some(item=>!brokenGallery.includes(item.id))&&<section className="card"><small>PHOTOS</small><h2>Restaurant gallery</h2><div className="publicGallery">{gallery.filter(item=>!brokenGallery.includes(item.id)).map(item=><figure className="publicGalleryItem" key={item.id}><a className="publicGalleryLink" href={item.public_url} target="_blank" rel="noopener noreferrer" referrerPolicy="no-referrer" aria-label={`Open ${item.title||restaurant.name} photo in full size`}><Image src={item.public_url} alt={item.title||item.caption||`${restaurant.name} gallery photo`} width={800} height={600} sizes="(max-width: 520px) 100vw, (max-width: 800px) 50vw, 33vw" loading="lazy" onError={()=>setBrokenGallery(ids=>ids.includes(item.id)?ids:[...ids,item.id])} /></a>{item.caption&&<figcaption>{item.caption}</figcaption>}</figure>)}</div></section>}
 <section className="card"><small>LOCATION</small><h2>Find us</h2><div className="location"><div aria-hidden="true">⌖</div><p><strong>{restaurant.city}</strong><br/>{restaurant.address||"Address available on request."}</p><div className="locationActions"><a className="secondary locationAction" href={maps} target="_blank" rel="noopener noreferrer" referrerPolicy="no-referrer" onClick={()=>void track("directions")}>Directions</a>{addressText&&<button type="button" className="secondary locationAction" onClick={()=>void copyAddress()}>Copy address</button>}</div></div>{addressMessage&&<div className="addressNotice" role="status" aria-live="polite">{addressMessage}</div>}</section>
 <section className="card"><small>HOURS</small><h2>Opening hours</h2>{DAYS.map(([key,label])=><div className={`hour${todayKey===key?" currentDay":""}`} key={key} aria-current={todayKey===key?"date":undefined}><span>{label}{todayKey===key&&<small className="todayLabel">Today</small>}</span><strong>{hours[key]||"Hours not provided"}</strong></div>)}</section></div>
 <aside className="right"><section className="card trustCard"><small>TRUST & VERIFICATION</small>{restaurant.claim_status==="verified"?<><h2>Business verified by DineUp</h2><p>The restaurant has completed DineUp business verification.</p></>:restaurant.claim_status==="verification_pending"?<><h2>Verification in progress</h2><p>Business verification is currently under review.</p></>:<><h2>Listed on DineUp</h2><p>Business verification information is not currently available.</p></>}</section><section className="card dark"><small>DINEUP VISIBILITY</small><h2>{sponsored?"Promoted on DineUp":"Listed on DineUp"}</h2>{sponsored&&<div className="bid">₹{bid.toLocaleString("en-IN")}</div>}<p>{sponsored?"This restaurant is participating in the DineUp visibility marketplace.":"Discover this restaurant on the DineUp marketplace."}</p><Link href="/marketplace" className="lightButton">Explore marketplace</Link></section>{(phone||whatsappNumber)&&<section className="card"><small>CONTACT</small>{phone&&<div className="contactRow"><a href={`tel:${phone}`} className="contactPhone" onClick={()=>void track("call")}>{phone}</a><button type="button" className="secondary contactCopy" aria-label={`Copy ${restaurant.name} phone number`} title={`Copy ${restaurant.name} phone number`} onClick={()=>void copyPhone()}>Copy</button></div>}{whatsappNumber&&<a className="secondary contactWhatsapp" href={`https://wa.me/${whatsappNumber}?text=${whatsappMessage}`} target="_blank" rel="noopener noreferrer" referrerPolicy="no-referrer" onClick={()=>void track("whatsapp")}>WhatsApp</a>}{phoneMessage&&<div className="addressNotice" role="status" aria-live="polite">{phoneMessage}</div>}</section>}<section className="card"><small>QUICK INFO</small><div className="info"><span>Category</span><strong>{restaurant.category}</strong></div><div className="info"><span>City</span><strong>{restaurant.city}</strong></div>{profile?.price_range&&<div className="info"><span>Price range</span><strong>{profile.price_range}</strong></div>}{profile?.cuisine_tags?.length&&<div className="cuisineInfo"><span>Cuisine</span><div className="cuisineTags">{profile.cuisine_tags.map(tag=><span className="cuisineTag" key={tag}>{tag}</span>)}</div></div>}</section></aside></div></div></section><div className="mobileStickyCta" role="group" aria-label="Restaurant contact actions">{phoneHref&&<a href={`tel:${phoneHref}`} className="mobileCtaPrimary" aria-label={`Call ${restaurant.name}`} onClick={()=>void track("call")}>Call</a>}{whatsappNumber&&<a href={`https://wa.me/${whatsappNumber}?text=${whatsappMessage}`} className="mobileCtaSecondary" target="_blank" rel="noopener noreferrer" referrerPolicy="no-referrer" aria-label={`Chat with ${restaurant.name} on WhatsApp`} onClick={()=>void track("whatsapp")}>WhatsApp</a>}<a href={maps} className="mobileCtaSecondary" target="_blank" rel="noopener noreferrer" referrerPolicy="no-referrer" aria-label={`Get directions to ${restaurant.name}`} onClick={()=>void track("directions")}>Directions</a></div><footer>DineUp • Where Restaurants Rise</footer><style jsx global>{styles}</style></main>;
}
