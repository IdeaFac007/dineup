...
export default function PublicRestaurantProfile(){
 ...
 return <main className="page"><Header/><section className="shell"><nav className="breadcrumb" aria-label="Breadcrumb"><Link href="/marketplace">Marketplace</Link><span aria-hidden="true">/</span><span>{restaurant.city}</span><span aria-hidden="true">/</span><span aria-current="page">{restaurant.name}</span></nav><div className="cover">{profile?.cover_image_url&&!brokenCover?<Image className="coverImage" src={profile.cover_image_url} alt={`${restaurant.name} cover`} fill priority sizes="(max-width: 800px) 100vw, 1180px" referrerPolicy="no-referrer" onError={()=>setBrokenCover(true)} />:<div className="cover-letter">{restaurantInitial}</div>}<div className="coverShade"/><div className="rankBadge">{rank?`#${rank} in ${restaurant.city}`:"DineUp listing"}</div></div>
