...
export default function RestaurantDashboard() {
  ...
  async function uploadProfileImage(kind: "logo" | "cover", file: File) {
    ...
    try {
      ...
      setProfileMessage(e?.message || "Unable to upload image right now.");
    } finally {
  ...
  async function saveProfile() {
    ...
    try {
      ...
      setProfileMessage(e?.message || "Unable to save profile right now.");
    } finally {
  ...
  const loadDashboard = useCallback(async (isRefresh = false) => {
    ...
    try {
      ...
      const { data: myRestaurant, error: restaurantError } = await supabase.from("restaurants")
        .select("id, name, city, category, address, current_bid, is_active")
      ...
      const r = { ...myRestaurant, id: Number(myRestaurant.id), current_bid: Number(myRestaurant.current_bid || 0) };
      setRestaurant(r);

      const { data: p } = await supabase.from("restaurant_profiles")
        .select("phone, whatsapp, website_url, description, price_range, menu_url, cover_image_url, logo_image_url, opening_hours")
        .eq("restaurant_id", r.id).maybeSingle();
      ...
      const { data: restaurants, error: leaderboardError } = await supabase.from("restaurants")
        .select("id, name, current_bid").eq("is_active", true).eq("city", r.city).order("current_bid", { ascending: false });
      if (leaderboardError) throw leaderboardError;
      const lb = (restaurants || []).map((x: any) => ({ ...x, id: Number(x.id), current_bid: Number(x.current_bid || 0) }));
      const idx = lb.findIndex((x: any) => x.id === r.id);
      if (idx >= 0) {
        setRank(idx + 1);
      ...
      const { data: bidData } = await supabase.from("bids")
        .select("id, amount, status, payment_status, razorpay_order_id, razorpay_payment_id, created_at")
        .eq("restaurant_id", r.id).order("created_at", { ascending: false }).limit(20);
  ...
  async function handleLogout() {
    setLoggingOut(true); const { error } = await supabase.auth.signOut();
  ...
  return (
    <main className="page"><style jsx>{styles}</style>
