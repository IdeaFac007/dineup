  ...
  return <main className="page"><style jsx>{styles}</style>
    ...
    <div className="shell">
      ...
      <div className="grid">
        ...
        <section className="panel"><div className="panelTitle"><b>Restaurant profile</b><div className="titleActions"><span className="status">LIVE</span><button className="secondary small" onClick={() => { setProfileMessage(""); setProfileOpen(true); }}>Edit profile</button></div></div><div className="summary"><div><small>Restaurant</small><b>{restaurant.name}</b></div><div><small>Location</small><b>{restaurant.city}</b>{restaurant.address && <p>{restaurant.address}</p>}</div><div><small>Category</small><b>{restaurant.category}</b></div></div><div className="actions"><Link href="/marketplace" className="secondary">View marketplace</Link><Link href={`/restaurant/bid?id=${restaurant.id}`} className="primary">Promote restaurant</Link></div></section>
      </div>
