...
export default function RestaurantDashboard() {
  ...
  return <main className="page"><style jsx>{styles}</style>
    ...
    <div className="shell">
      ...
      <section className="panel onboardingPanel"><div className="panelTitle"><div><b>Restaurant onboarding</b><p className="muted panelSub">Complete the key steps to make your listing customer-ready.</p></div><span className="completionBadge">{Number(restaurant.profile_completion_pct||0)}% profile</span></div><div className="progressTrack"><div className="progressFill" style={{width:`${Math.min(100,Math.max(0,Number(restaurant.profile_completion_pct||0)))}%`}}/></div><div className="progressMeta"><span>Profile completion</span><b>{Number(restaurant.profile_completion_pct||0)}%</b></div><div className="onboardingSteps">{onboarding.map((item:any)=><div className={`onboardingStep ${item.completed?"done":""}`} key={item.step}><div className="stepIcon">{item.completed?"✓":"•"}</div><div className="stepBody"><b>{item.label}</b><span>{item.completed?"Completed":"Pending"}</span></div>{item.completed?<span className="stepStatus">DONE</span>:<Link className="secondary small" href={item.action_url}>{item.step==="payment"?"Promote":"Complete"}</Link>}</div>)}</div></section>
