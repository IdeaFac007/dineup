...
export default function RestaurantReviewsPage(){
 ...
 return <main className="page"><style jsx>{css}</style>
  ...
  <div className="shell">
   ...
   <div className="stats"><div><span>Average rating</span><strong>{average?average.toFixed(1):"—"}</strong></div><div><span>Total reviews</span><strong>{reviews.length}</strong></div><div><span>5-star reviews</span><strong>{reviews.filter(x=>x.rating===5).length}</strong></div><div className="distributionCard"><span>Rating distribution</span>{[5,4,3,2,1].map(n=><div className="distRow" key={n}><b>{n}★</b><i><em style={{width:`${reviews.length?Math.round((reviews.filter(x=>x.rating===n).length/reviews.length)*100):0}%`}} /></i><small>{reviews.filter(x=>x.rating===n).length}</small></div>)}</div></div>
