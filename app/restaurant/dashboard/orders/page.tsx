...
export default function RestaurantOrdersPage() {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on(
        ...
        (payload) => {
          if (!active) return;
          const changedOrder = payload.new as Partial<Order>;
          const eventLabel = payload.eventType === "INSERT"
            ? "New order received."
            : payload.eventType === "UPDATE"
              ? "Order updated."
              : "Order removed.";
          const text = changedOrder.order_number ? `${eventLabel} ${changedOrder.order_number}` : eventLabel;
          setMessage(text);
          if (document.hidden && Notification.permission === "granted") {
            new Notification("DineUp restaurant order update", { body: text });
          }
  ...
  }, [restaurant?.id, supabase, loadOrders]);

  useEffect(() => {
    if (typeof Notification !== "undefined" && Notification.permission === "granted") setAlerts(true);
  }, []);
  useEffect(() => {
    if (!message) return;
        ...
        <div className="heading">
          <div>
            ...
            <div className="titleRow">
              <h1>Orders</h1>
              <span className="live"><i /> Live updates</span>{!alerts && <button className="secondary alertButton" onClick={() => void enableAlerts()}>Enable alerts</button>}
            </div>
            <p className="muted">{restaurant?.name} · New customer orders and status changes appear automatically.</p>
          </div>
          <Link href="/restaurant/dashboard" className="secondary">← Dashboard</Link>
        </div>
