...
export default function GrowthPage() {
  ...
  return <main className="wrap">
    <div className="head"><div><div className="eyebrow">DINEUP GROWTH ENGINE</div><h1>Growth Dashboard</h1><p>Revenue, acquisition and marketplace activity in one view.</p></div><div className="actions"><select value={range} onChange={e=>setRange(Number(e.target.value) as 7|30|90)}><option value={7}>Last 7 days</option><option value={30}>Last 30 days</option><option value={90}>Last 90 days</option></select><button onClick={()=>router.push("/admin/dashboard")}>← Admin</button></div></div>
    {error && <div className="error">{error}</div>}
    <section className="grid">
      <Card title="Net Revenue" value={money(net)} note={`${money(gross)} gross · ${money(refunds)} refunds`}/>
      <Card title="Paid Bids" value={String(captured.length)} note={`${recentBids.length} total bids`}/>
      <Card title="Bid → Payment" value={`${bidToPaid.toFixed(1)}%`} note="Captured bids / all bids"/>
      <Card title="Application Conversion" value={`${appConversion.toFixed(1)}%`} note={`${appApproved} approved / ${recentApps.length} applications`}/>
    </section>
    <section className="grid two">
      <Panel title="Marketplace Activity"><div className="metrics"><Metric label="Active restaurants" value={restaurants.filter(r=>r.is_active!==false).length}/><Metric label="Claimed restaurants" value={restaurants.filter(r=>r.is_claimed===true).length}/><Metric label="Customer actions" value={recentEvents.length}/><Metric label="Profile views" value={recentEvents.filter(e=>e.event_type==='profile_view').length}/></div></Panel>
      <Panel title="Revenue by City"><div className="table">{cityRows.length ? cityRows.map(r=><div className="row" key={r.city}><strong>{r.city}</strong><span>{r.restaurants} restaurants · {r.bids} bids</span><b>{money(r.revenue)}</b></div>) : <p>No city activity yet.</p>}</div></Panel>
    </section>
    ...
    <div className="footer">Phase 8.1 · Growth intelligence layer · Data refreshes on page load</div>
    <style jsx>{css}</style>
  </main>;
}
