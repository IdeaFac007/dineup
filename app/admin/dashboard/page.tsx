...
export default function AdminDashboardPage() {
  ...
  async function updateRestaurant(id: number, field: "is_active" | "is_claimed", value: boolean) {
    setActionLoading(id);
    setError("");
    ...
    try {
      ...
      if (updateError) {
        setError(updateError.message || "Unable to update restaurant.");
        return;
      }
      ...
      console.error(err);
      setError("Something went wrong while updating the restaurant.");
    } finally {
      setActionLoading(null);
    }
  }
  ...
  const stats = useMemo(() => {
    ...
    const fullApplications = applications.filter((a) => a.onboarding_mode !== "bid_only").length;
    const bidOnlyApplications = applications.filter((a) => a.onboarding_mode === "bid_only").length;
    const staleApplications = applications.filter((a) => a.status === "pending" && Date.now() - new Date(a.created_at).getTime() > 48 * 60 * 60 * 1000).length;
    const capturedAmount = captured.reduce((sum, b) => sum + Number(b.amount || 0), 0);
    const failedByRestaurant = new Map<number, number>();
    failed.forEach((b) => failedByRestaurant.set(b.restaurant_id, (failedByRestaurant.get(b.restaurant_id) || 0) + 1));
    const repeatedPaymentFailures = Array.from(failedByRestaurant.values()).filter((count) => count >= 3).length;
    const recentBidCounts = new Map<number, number>();
    const cutoff = Date.now() - 15 * 60 * 1000;
    bids.forEach((b) => { if (new Date(b.created_at).getTime() >= cutoff) recentBidCounts.set(b.restaurant_id, (recentBidCounts.get(b.restaurant_id) || 0) + 1); });
    const rapidBidActivity = Array.from(recentBidCounts.values()).filter((count) => count >= 5).length;
    return { total, active, claimed, totalBidValue, highestBid, cities, totalBids: bids.length, capturedPayments: captured.length, pendingPayments: pending.length, failedPayments: failed.length, awaitingPayments: awaiting.length, capturedAmount, pendingApplications, fullApplications, bidOnlyApplications, staleApplications, repeatedPaymentFailures, rapidBidActivity };
          ...
          <main className="content">
            ...
            {activeNav === "overview" && <>
              ...
              <section className="dashboardGrid">
                <div className="panel"><div className="panelHeader"><div><h3>Live Leaderboard</h3><p>{leaderboardCity === "All" ? "Select a city to view its local bid ranking." : `Restaurants ranked by current bid in ${leaderboardCity}.`}</p></div><div className="panelHeaderActions"><select className="leaderboardCity" aria-label="Leaderboard city" value={leaderboardCity} onChange={(e) => setLeaderboardCity(e.target.value)}><option value="All">All cities</option>{Array.from(new Set(restaurants.map((r) => r.city).filter(Boolean))).sort().map((city) => <option key={city} value={city}>{city}</option>)}</select><button className="textButton" onClick={() => setActiveNav("leaderboard")}>View all →</button></div></div><div className="leaderboard">{leaderboardRestaurants.length ? leaderboardRestaurants.map((r, i) => <div className="leaderRow" key={r.id}><div className={i === 0 ? "rank first" : "rank"}>{i + 1}</div><div className="restaurantAvatar">{r.name.charAt(0).toUpperCase()}</div><div className="restaurantInfo"><strong>{r.name}</strong><span>{r.category} • {r.city}</span></div><div className="bidAmount">₹{Number(r.current_bid || 0).toLocaleString("en-IN")}</div><div className="statusPill">{r.is_active !== false ? "Active" : "Inactive"}</div></div>) : <EmptyState/>}</div></div>
              ...
              <section className="monitorSection"><div className="sectionTitle"><h3>Admin & Abuse Monitor</h3><p>Read-only signals for unusual production activity.</p></div><div className="monitorGrid"><div className={stats.repeatedPaymentFailures ? "monitorCard alert" : "monitorCard"}><strong>{stats.repeatedPaymentFailures}</strong><span>Restaurants with 3+ failed payments</span></div><div className={stats.rapidBidActivity ? "monitorCard alert" : "monitorCard"}><strong>{stats.rapidBidActivity}</strong><span>Restaurants with 5+ bids in 15 min</span></div><div className="monitorCard"><strong>{stats.failedPayments}</strong><span>Failed payment records</span></div><button className="monitorCard linkCard" onClick={() => router.push("/admin/audit")}><strong>Audit →</strong><span>Review authorized admin actions</span></button></div></section><section className="quickSection"><div className="sectionTitle"><h3>Quick Management</h3><p>Jump directly to important admin sections.</p></div><div className="quickGrid"><QuickAction icon="✓" title="Applications" description={`${stats.pendingApplications} pending review`} onClick={() => setActiveNav("applications")}/><QuickAction icon="▣" title="Verification" description="Review business documents" onClick={() => router.push("/admin/verification")}/><QuickAction icon="◉" title="Restaurants" description="Manage restaurant listings" onClick={() => setActiveNav("restaurants")}/><QuickAction icon="♛" title="Leaderboard" description="Monitor ranking positions" onClick={() => setActiveNav("leaderboard")}/><QuickAction icon="↗" title="Bids" description={`${stats.totalBids} bids recorded`} onClick={() => setActiveNav("bids")}/><QuickAction icon="₹" title="Payments" description={`${stats.capturedPayments} captured`} onClick={() => setActiveNav("payments")}/><QuickAction icon="⚠" title="Abuse Monitor" description={`${stats.repeatedPaymentFailures + stats.rapidBidActivity} alerts to review`} onClick={() => router.push("/admin/audit")}/></div></section>
            </>}
            ...
            {activeNav === "applications" && (
              <section className="panel fullPanel">
                ...
                <div className="applicationSummary">
                  ...
                  <MiniStat label="Total Applications" value={applications.length}/><MiniStat label="Bid-Only" value={stats.bidOnlyApplications}/><MiniStat label="Full Listing" value={stats.fullApplications}/><MiniStat label="Pending >48h" value={stats.staleApplications}/>
                </div>
                <ApplicationList
...
function RestaurantTable({
  ...
  return (
    <div className={showRank ? "restaurantList leaderboardList" : "restaurantList"}>
      {restaurants.map((r, i) => {
        const busy = actionLoading === r.id;
        const restaurantBids = bids.filter((b) => b.restaurant_id === r.id);
        const lastActivity = restaurantBids.length
          ? restaurantBids.reduce((latest, b) => new Date(b.created_at) > new Date(latest) ? b.created_at : latest, restaurantBids[0].created_at)
          : r.created_at;

        return (
          <article className="restaurantCard" key={r.id}>
            <div className="restaurantCardMain">
              {showRank && <div className={i === 0 ? "restaurantRank first" : "restaurantRank"}>#{i + 1}</div>}
              <div className="tableAvatar largeAvatar">{r.name.charAt(0).toUpperCase()}</div>
              <div className="restaurantCardIdentity">
                <strong>{r.name}</strong>
                <span>{r.category} • {r.city}</span>
                {!showRank && <small>{r.address || "Address not added"}</small>}
              </div>
            </div>

            <div className="restaurantCardStats">
              <div><span>Current bid</span><strong>₹{Number(r.current_bid || 0).toLocaleString("en-IN")}</strong></div>
              {!showRank && <div><span>Total bids</span><strong>{restaurantBids.length}</strong></div>}
              {!showRank && <div><span>Owner</span><strong>{r.owner_id ? "Linked" : "Unlinked"}</strong><small>{r.owner_id ? `${r.owner_id.slice(0, 8)}…` : "No owner"}</small></div>}
              {!showRank && <div><span>Last activity</span><strong>{lastActivity ? formatDate(lastActivity) : "—"}</strong></div>}
            </div>

            <div className="restaurantCardStatus">
              <div className="statusStack">
                <span className="statusLabel">Status</span>
                {r.is_active !== false ? <span className="successBadge">Active</span> : <span className="neutralBadge">Inactive</span>}
              </div>
              {!showRank && <div className="statusStack">
                <span className="statusLabel">Claim</span>
                {r.is_claimed ? <span className="infoBadge">Claimed</span> : <span className="neutralBadge">Unclaimed</span>}
              </div>}
            </div>
            ...
            {!showRank && (
              <div className="restaurantCardActions">
                ...
                <button className="tableAction secondary" disabled={busy} onClick={() => onToggleClaim?.(r)}>
                  {r.is_claimed ? "Unclaim" : "Claim"}
                </button>
              </div>
            )}
          </article>
      ...
      {applications.map((application) => {
        ...
        return (
          <article className="applicationCard" key={application.id}>
            ...
            <div className="applicationMode"><span className={application.onboarding_mode === "bid_only" ? "modeBadge bidOnly" : "modeBadge"}>{application.onboarding_mode === "bid_only" ? "BID-ONLY" : "FULL LISTING"}</span></div>
...
@media(max-width:760px){.applicationCard{grid-template-columns:1fr;gap:13px}.applicationMeta{grid-template-columns:1fr 1fr}.applicationSummary{grid-template-columns:1fr}.applicationDetails{grid-template-columns:1fr}.reviewActions{flex-direction:column}.reviewActions button{width:100%}}`;
