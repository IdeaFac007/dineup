...
function RestaurantTable({
  ...
  return (
    <div className="tableScroll">
      <table className="dataTable restaurantTable">
        <thead>
          <tr>
            {showRank && <th>Rank</th>}
            <th>Restaurant</th>
            ...
            <th>Category</th>
            <th>Current Bid</th>
            {!showRank && <th>Total Bids</th>}
            {!showRank && <th>Owner</th>}
            {!showRank && <th>Address</th>}
            {!showRank && <th>Last Activity</th>}
            <th>Status</th>
            <th>Claim</th>
            {!showRank && <th>Actions</th>}
          </tr>
        </thead>
        <tbody>
          {restaurants.map((r, i) => {
            ...
            return (
              <tr key={r.id}>
                ...
                <td><TableRestaurant restaurant={r}/></td>
                <td>{r.city}</td>
                <td>{r.category}</td>
                <td className="tableBid">₹{Number(r.current_bid || 0).toLocaleString("en-IN")}</td>
                {!showRank && <td>
                  <div className="restaurantMeta">
                    <strong>{bids.filter((b) => b.restaurant_id === r.id).length}</strong>
                    <span>bids</span>
                  </div>
                </td>}
                {!showRank && <td>
                  <div className="restaurantMeta">
                    <strong>{r.owner_id ? "Linked" : "Unlinked"}</strong>
                    <span>{r.owner_id ? `${r.owner_id.slice(0, 8)}…` : "No owner"}</span>
                  </div>
                </td>}
                {!showRank && <td className="addressCell">{r.address || "—"}</td>}
                {!showRank && <td className="dateCell">{lastActivity ? formatDate(lastActivity) : "—"}</td>}
                <td>{r.is_active !== false ? <span className="successBadge">Active</span> : <span className="neutralBadge">Inactive</span>}</td>
                <td>{r.is_claimed ? <span className="infoBadge">Claimed</span> : <span className="neutralBadge">Unclaimed</span>}</td>
                {!showRank && (
                  <td>
                    <div className="actionGroup">
                      ...
                      <button
                        className="tableAction secondary"
                        disabled={busy}
                        onClick={() => onToggleClaim?.(r)}
                      >
