...
export default function MarketplacePage(){
  ...
  async function loadRestaurants(){
    ...
    try{
      ...
      if(rows.length){
        ...
        const {data:profiles}=await supabase.from("restaurant_profiles").select("restaurant_id,phone,whatsapp,website_url,description,price_range,menu_url,cover_image_url,logo_image_url").in("restaurant_id",ids);
  ...
  return <main className="market">
    ...
    <section className="hero"><div className="heroInner">
      <div className="eyebrow">INDIA'S RESTAURANT MARKETPLACE</div>
      <h1>Discover places<br/><em>worth dining at.</em></h1>
      <p>Explore trusted restaurants, see live marketplace bids, and connect directly with the places you love.</p>
      <div className="searchbox"><span>⌕</span><input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search restaurants, cuisine or location..."/><button onClick={()=>document.getElementById("restaurants")?.scrollIntoView({behavior:"smooth"})}>Search</button></div>
      <div className="trust"><span>✓ Verified restaurants</span><span>•</span><span>Live bidding</span><span>•</span><span>Direct contact</span></div>
    </div></section>

    <section className="controls" id="restaurants"><div className="controlInner"><div><span className="sectionKicker">EXPLORE</span><h2>Restaurants near you</h2></div><div className="filters"><select value={city} onChange={e=>setCity(e.target.value)}>{cities.map(c=><option key={c}>{c}</option>)}</select><select value={category} onChange={e=>setCategory(e.target.value)}>{categories.map(c=><option key={c}>{c}</option>)}</select></div></div></section>

    {error&&<div className="error">{error}</div>}
    <section className="content">
      {loading?<div className="loading">Finding restaurants...</div>:<>
        {top.length>0&&<><div className="featuredHead"><div><span className="sectionKicker">LIVE MARKETPLACE</span><h2>Featured restaurants</h2></div><span className="count">{filtered.length} places</span></div>
        <div className="cards">{top.map((r,i)=><article className="restaurantCard" key={r.id} role="link" tabIndex={0} onClick={()=>openRestaurant(r)} onKeyDown={e=>{if(e.key==="Enter"||e.key===" "){e.preventDefault();openRestaurant(r)}}}>
          ...
          <div className="cardBody"><div className="cardTop"><div><div className="tag">{i===0?"TRENDING":"FEATURED"}</div><h3>{r.name}</h3><p>{r.city} <b>·</b> {r.category}</p></div>{r.is_claimed&&<span className="verified">✓ Verified</span>}</div>
          ...
          <div className="actions">
            ...
            {r.profile?.whatsapp&&<a href={`https://wa.me/${r.profile.whatsapp.replace(/\D/g,"")}`} onClick={e=>{e.stopPropagation();action(r,"whatsapp")}}>WhatsApp</a>}
        ...
        <div className="list">{filtered.slice(3).map(r=><Link className="listItem" key={r.id} href={`/restaurant/${r.id}`} onClick={()=>void trackMarketingEvent("restaurant_view",r.id)}><div className="miniLogo">{r.profile?.logo_image_url?<img src={r.profile.logo_image_url} alt=""/>:r.name.slice(0,1)}</div><div className="listInfo"><strong>{r.name}</strong><span>{r.city} · {r.category}</span></div><div className="listBid"><span>Live bid</span><b>{money(r.current_bid)}</b></div><span className="arrow">→</span></Link>)}
