...
export async function POST(request: Request) {
  try {
    ...
    if (!Number.isInteger(bidId) || bidId <= 0) return NextResponse.json({ error: "Invalid bid ID." }, { status: 400 });
    if (requestedAmount !== null && (!Number.isFinite(requestedAmount) || requestedAmount <= 0)) return NextResponse.json({ error: "Invalid refund amount." }, { status: 400 });

    const keyId = process.env.RAZORPAY_KEY_ID;
    const keySecret = process.env.RAZORPAY_KEY_SECRET;
    if (!keyId || !keySecret) return NextResponse.json({ error: "Razorpay server configuration is missing." }, { status: 500 });
    ...
    if (bid.payment_status !== "captured" || !bid.razorpay_payment_id) return NextResponse.json({ error: "Only captured payments can be refunded." }, { status: 400 });

    const paymentResponse = await fetch(`https://api.razorpay.com/v1/payments/${encodeURIComponent(bid.razorpay_payment_id)}`, {
      headers: { Authorization: basicAuth(keyId, keySecret) },
      cache: "no-store",
    });
    const payment = await paymentResponse.json();
    if (!paymentResponse.ok) return NextResponse.json({ error: "Unable to verify the payment with Razorpay." }, { status: 502 });
    if (payment.status !== "captured" || payment.currency !== "INR") return NextResponse.json({ error: "The payment is not refundable in its current state." }, { status: 400 });
    ...
    if (!Number.isFinite(remainingPaise) || remainingPaise <= 0) return NextResponse.json({ error: "This payment has already been fully refunded." }, { status: 400 });
    ...
    if (!refundResponse.ok) {
      await supabase.rpc("record_admin_audit", {
        ...
        p_entity_type: "bid",
        p_entity_id: String(bid.id),
        p_amount: requestedAmount,
        p_success: false,
        p_metadata: { restaurant_id: String(bid.restaurant_id), payment_id: bid.razorpay_payment_id, reason },
      });
    ...
    await supabase.rpc("record_admin_audit", {
      ...
      p_metadata: {
        ...
        payment_id: bid.razorpay_payment_id,
    ...
    if (recordError) {
      return NextResponse.json({ success: true, warning: "Refund created at Razorpay but DineUp reconciliation is pending.", refundId: refund.id, status: refund.status });
    }

    return NextResponse.json({ success: true, refundId: refund.id, status: refund.status, amount: refundAmount });
  } catch (error) {
