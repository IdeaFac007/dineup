...
export default function RestaurantReviews({restaurantId}:{restaurantId:number}){
 ...
 async function editReview(review:Review){
  const nextBody=window.prompt("Update your review:",review.body||""); if(nextBody===null)return;
  const clean=nextBody.replace(/\s+/g," ").trim().slice(0,1000); if(!clean){setError("Review cannot be empty.");return}
  const {error:e}=await supabase.from("restaurant_reviews").update({body:clean}).eq("id",review.id).eq("customer_id",userId); if(e)setError(e.message); else {setMessage("Review updated.");await load()}
 }
 async function deleteReview(review:Review){
  if(!window.confirm("Delete this review? This cannot be undone."))return;
  const {error:e}=await supabase.from("restaurant_reviews").delete().eq("id",review.id).eq("customer_id",userId); if(e)setError(e.message); else {setMessage("Review deleted.");await load()}
 }
 ...
 return <section id="reviews" className="reviews">
  <div className="reviewsHead"><div><span className="eyebrow">CUSTOMER TRUST</span><h2>Reviews & ratings</h2><p>Verified reviews from customers who completed a paid order.</p></div><div className="summary"><strong>{average?average.toFixed(1):"—"}</strong><span>{average?stars(Math.round(average)):"☆☆☆☆☆"}</span><small>{reviews.length} review{reviews.length===1?"":"s"}</small><div className="distribution">{[5,4,3,2,1].map(n=><div key={n}><b>{n}★</b><i><em style={{width:`${reviews.length?Math.round((reviews.filter(x=>x.rating===n).length/reviews.length)*100):0}%`}} /></i><small>{reviews.filter(x=>x.rating===n).length}</small></div>)}</div></div></div>
  ...
  {!loading&&!reviews.length?<div className="empty">No reviews yet. Be the first verified customer to share your experience.</div>:<div className="list">{reviews.map(r=><article key={r.id} className="review"><div className="reviewTop"><div><strong>{stars(r.rating)}</strong>{r.title&&<h3>{r.title}</h3>}<small><span className="verifiedBadge">✓ Verified purchase</span> · {new Date(r.created_at).toLocaleDateString("en-IN",{day:"2-digit",month:"short",year:"numeric"})}</small></div></div>{r.body&&<p>{r.body}</p>}<button className="report" type="button" disabled={reporting===r.id||reported.has(r.id)} onClick={()=>void reportReview(r.id)}>{reported.has(r.id)?"Reported":"Report review"}</button>{r.customer_id===userId&&<span className="ownerActions"><button type="button" onClick={()=>void editReview(r)}>Edit</button><button type="button" onClick={()=>void deleteReview(r)}>Delete</button></span>}{r.owner_response&&<div className="response"><b>Restaurant response</b><p>{r.owner_response}</p></div>}</article>)}</div>}
  <style jsx>{css}</style>
 </section>
}
const css=`
.reviews{margin-top:28px;background:#fff;border:1px solid #e5e5e5;border-radius:20px;padding:24px}.reviewsHead{display:flex;justify-content:space-between;gap:20px;align-items:flex-start}.eyebrow{font-size:9px;letter-spacing:2px;font-weight:900;color:#777}.reviews h2{margin:7px 0 5px;font-size:24px}.reviewsHead p,.write small,.review small{display:block;color:#777;font-size:11px}.summary{text-align:right;display:grid;gap:3px}.summary strong{font-size:30px}.summary span{font-size:14px;letter-spacing:1px}.summary small{color:#777;font-size:9px}.distribution{margin-top:8px;display:grid;gap:4px;text-align:left}.distribution>div{display:grid;grid-template-columns:24px 100px 20px;align-items:center;gap:5px;font-size:8px}.distribution i{height:5px;background:#eee;border-radius:99px;overflow:hidden}.distribution em{display:block;height:100%;background:#111}.write{margin:20px 0;padding:16px;border:1px solid #eee;border-radius:14px;background:#fafafa;display:grid;gap:10px}.stars{display:flex;gap:3px}.stars button{border:0;background:transparent;font-size:24px;color:#ccc;cursor:pointer;padding:0}.stars button.selected{color:#111}.write input,.write textarea{border:1px solid #ddd;border-radius:9px;padding:10px;font:inherit;font-size:12px;background:#fff}.submit{justify-self:start;border:0;border-radius:9px;background:#111;color:#fff;padding:10px 14px;font-size:10px;font-weight:900;cursor:pointer}.submit:disabled{opacity:.5}.notice{padding:10px;border-radius:9px;font-size:10px;margin-top:12px}.success{background:#eefaf3;color:#176b3c}.error{background:#fff0f0;color:#a22}.list{display:grid;gap:12px;margin-top:16px}.review{border-top:1px solid #eee;padding-top:14px}.verifiedBadge{display:inline-flex;align-items:center;border:1px solid #d7e9dc;background:#f2fbf4;color:#176b3c;border-radius:999px;padding:3px 7px;font-size:8px;font-weight:900;margin-right:4px}.reviewTop strong{font-size:13px;letter-spacing:1px}.review h3{margin:5px 0 2px;font-size:14px}.review p{font-size:12px;line-height:1.6;color:#333}.ownerActions{margin-left:10px;display:inline-flex;gap:8px}.ownerActions button{border:0;background:transparent;color:#777;font-size:9px;text-decoration:underline;cursor:pointer}.report{margin-top:8px;border:0;background:transparent;color:#777;font-size:9px;padding:0;cursor:pointer;text-decoration:underline}.report:disabled{opacity:.5;cursor:default}.response{margin-top:10px;padding:10px;border-left:3px solid #ddd;background:#fafafa}.response p{margin:5px 0 0}.empty{margin-top:15px;padding:18px;background:#fafafa;border-radius:12px;color:#777;font-size:11px}@media(max-width:650px){.reviews{padding:18px}.reviewsHead{display:block}.summary{text-align:left;margin-top:12px}.submit{width:100%}}
`;
