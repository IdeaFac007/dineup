...
export default function MarketplacePage(){
  ...
  return <main className="market">
    <nav className="nav">
      ...
      <div className="navRight"><label className="navLocation" aria-label="Choose city"><span>⌖</span><select value={city==="All"?"Lucknow":city} onChange={e=>setCity(e.target.value)}>{cities.filter(c=>c!=="All").map(c=><option key={c} value={c}>{c}</option>)}</select></label><Link href="/restaurant/apply" className="partner">List your restaurant <span>→</span></Link><Link href="/hub" className="navUser" aria-label="Account" title="Your DineUp account"><svg viewBox="0 0 24 24" aria-hidden="true"><circle cx="12" cy="8" r="3.5"/><path d="M5 20c.7-3.6 3.2-5.5 7-5.5s6.3 1.9 7 5.5"/></svg></Link></div>
    </nav>

    <section className="hero"><div className="heroInner">
      <div className="heroCopy">
        <div className="eyebrow">🍽 INDIA'S RESTAURANT DISCOVERY PLATFORM</div><div className="marketplacePreviewBadge"><span>●</span> LIVE MARKETPLACE · OWNER BIDDING OPEN</div>
    ...
    <section className="statsBar"><div className="statsInner">
      ...
      <div><b>⌖</b><span><strong>{activeCityCount}</strong><small>Cities</small></span></div>
    ...
    <section className="controls" id="restaurants"><div className="controlInner"><div><span className="sectionKicker">EXPLORE</span><h2>Find your next restaurant</h2></div><div className="filters"><select aria-label="Filter by city" value={city} onChange={e=>setCity(e.target.value)}>{cities.map(c=><option key={c}>{c}</option>)}</select><select aria-label="Filter by cuisine category" value={category} onChange={e=>setCategory(e.target.value)}>{categories.map(c=><option key={c}>{c}</option>)}</select><select aria-label="Sort restaurants" value={sort} onChange={e=>setSort(e.target.value)}><option value="recommended">Recommended</option><option value="bid">Most visible</option><option value="name">A–Z</option></select><label className="check"><input type="checkbox" aria-label="Show verified restaurants only" checked={verifiedOnly} onChange={e=>setVerifiedOnly(e.target.checked)}/> Verified only</label></div></div></section>
    ...
    <section className="content">
      {loading?<div className="loading" role="status" aria-live="polite">Finding restaurants...</div>:<>
        ...
        <div className="cards">{top.map((r,i)=>{
          ...
          const coverImageUrl=imageUrl(r.profile?.cover_image_url);
          return <article className="restaurantCard" key={r.id}>
          <Link className="cardClickHint" href={`/restaurant/${r.id}`} aria-label={`View ${r.name}`} onClick={()=>trackRestaurantView(r)}>View restaurant <span>↗</span></Link><button type="button" className={`favoriteBtn cardFavorite ${favoriteIds.has(r.id)?"saved":""}`} aria-label={favoriteIds.has(r.id)?"Remove from favourites":"Save restaurant"} onClick={()=>void toggleFavorite(r)}>{favoriteIds.has(r.id)?"♥":"♡"}</button>
          {coverImageUrl?<img className="cover" src={coverImageUrl} alt="" loading="lazy" decoding="async"/>:<div className="cover placeholder"><span>{r.name.slice(0,1).toUpperCase()}</span></div>}
          <div className="cardBody"><div className="cardTop"><div><div className="tag">{Number(r.current_bid||0)>0?"SPONSORED":i===0?"TRENDING":"FEATURED"}</div><h3>{r.name}</h3><p>{r.city} <b>·</b> {r.category}{r.profile?.cuisine_tags?.length ? <> <b>·</b> {r.profile.cuisine_tags.slice(0,2).join(", ")}</> : null}</p></div>{r.claim_status==="verified"&&<span className="verified">✓ Verified</span>}</div>
          <p className="description">{r.profile?.description||r.address||"Discover this restaurant on DineUp."}</p>
          <div className="bid"><span>{Number(r.current_bid||0)>0?"Current marketplace bid":"Opening marketplace bid"}</span><strong>{Number(r.current_bid||0)>0?money(r.current_bid):"₹0 · Be the first bidder"}</strong></div>
          <div className="actions">
            ...
            {r.claim_status==="unclaimed"&&<Link className="claimLink" href={`/restaurant/claim/${r.id}`} onClick={e=>e.stopPropagation()}>Claim this restaurant →</Link>}
          </div></div></article>})}</div></>}
